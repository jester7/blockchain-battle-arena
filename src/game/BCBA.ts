...
export default class BCBA {
  ...
  public postInit() {
    ...
    for (let i = 0; i < this.players.length; i++) {
      console.log("init handlers for player: ", i);

      this.players[i].getInputSource().initHandlers();
    }
    // this.players.forEach((player) => {
    //   // if (player === undefined) return;
    //   console.log("init handlers for player: ", player);
    //   player.getInputSource().initHandlers();
    // });
  }
